...
class CatalogView(ListView):
    ...
    def get_queryset(self):
        ...
        if category_slugs:
            subcategories_slugs = Category.objects.filter(parent__slug__in=category_slugs).values_list('slug', flat=True)
            category_slugs.extend(subcategories_slugs)
        ...
        if capacity_volumes:
            queryset = queryset.filter(
                Q(capacities__volume__in=capacity_volumes) & Q(capacities_perfumecapacity_available=True)
            ).distinct()
        ...
        if search_query:
            queryset = queryset.filter(
                ...
                Q(base_notes__name_icontains=search_query)
            ).distinct()
    ...
    def get_context_data(self, **kwargs):
        ...
        # Добавляем выбранную каьегорию (первую, если выбранно несколько)
...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        filter_categories = Category.objects.filter(show_in_filter=True)
